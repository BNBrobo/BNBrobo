...
contract BNBROBO {
	...
	uint constant public			INVEST_MIN_AMOUNT				= 0.05 ether;			// 0.05BNB
	uint[] public					REFERRAL_PERCENTS				= [100, 30, 20, 10, 10];	
	uint constant public			DEV_FEE							= 120;					// dev fee 12% of deposit
	uint constant public			MAX_WITHDRAW_AMOUNT				= 30 ether;				// claim 30 BNB max per time
	...
	uint256 public  startUnix = 1652967000;
	...
	function invest(address _referrer, uint8 _planIdx) public payable {
		...
		require(block.timestamp > startUnix,"Before time call");
